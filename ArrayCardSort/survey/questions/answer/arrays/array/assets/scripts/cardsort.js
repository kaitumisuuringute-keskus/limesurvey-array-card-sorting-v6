...
$(document).on('ready pjax:scriptcomplete', function () {
	...
	$('.card-sort-outer-wrapper').each(function (i) {
		...
		// Define some stuff...
		var thisQuestion = $(this).closest('.array-flexible-row');
		...
		if ($('#question' + qID + ' .card.draggable').length == 0) {
			...
			$('#question' + qID + ' .droppable').bind('drop', function (event, ui) {
				...
				$('#question' + qID + ' .question-item textarea, #question' + qID + ' .question-item input[type="text"], #question' + qID + ' .question-item input[type="hidden"], #question' + qID + ' .question-item select').val('');
				...
				$('#question' + qID + ' .question-item textarea, #question' + qID + ' .question-item input[type="text"], #question' + qID + ' .question-item input[type="hidden"], #question' + qID + ' .question-item select').trigger('keyup');
			});

			// Initial "card" positions
			$('#question' + qID + ' .question-item textarea, #question' + qID + ' .question-item input[type="text"], #question' + qID + ' .question-item input[type="hidden"], #question' + qID + ' .question-item select').each(function (i) {
				if ($.trim($(this).val()) != '') {
